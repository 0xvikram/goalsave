...
function CreateForm({ wallet, currentLedger, onCreated }) {
  ...
  // Approx ledgers from days; requires current ledger to be loaded first.
  const unlockOffset = Math.ceil(parseFloat(days || 1) * 86400 / 5)
  const unlockLedger = Number(currentLedger || 0) + unlockOffset
  const canBuildUnlock = Number(currentLedger) > 0 && Number.isFinite(unlockLedger)

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!wallet) return
    if (!canBuildUnlock) {
      setErr('Still loading latest ledger. Please wait a moment and try again.')
      return
    }
    setBusy(true); setErr('')
    try {
  ...
  return (
    <form className="create-form" onSubmit={handleSubmit}>
      ...
      <button type="submit" className="btn-create" disabled={!wallet || busy || !goalName || !canBuildUnlock}>
